# Try drawing a second container with another call to glDrawElements but place
...
proc draw() =
  ...
  # Container 2
  # Set transform matrix
  let s = GLfloat(sin(getTime()) * 0.25 + 0.75)  # map to [0.5, 1] range
  ...
  glUniformMatrix4fv(
    ...
    count = 1, transpose = false, transform.caddr)
...
proc main() =
  ...
  cfg.title = "05-Transformations/Exercise2"
  cfg.resizable = false
