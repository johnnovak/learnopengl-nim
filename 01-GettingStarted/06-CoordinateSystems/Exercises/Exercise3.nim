# Try to make every 3rd container (including the 1st) rotate over time, while
# leaving the other containers static using just the model matrix.
#
...
proc setup() =
  shaderProgram = createShaderProgramFromFile("exercise3.vs",
                                              "exercise3.fs")
...
proc draw() =
  ...
  glUniformMatrix4fv(
    ...
    count = 1, transpose = false, view.caddr)
  ...
  glUniformMatrix4fv(
    ...
    count = 1, transpose = false, projection.caddr)
  ...
  for i, pos in cubePositions.pairs:
    # Set model matrix
    var angle = degToRad(20.0 * float(i))
    if i mod 3 == 0:
      angle += getTime()
    ...
    glUniformMatrix4fv(
      ...
      count = 1, transpose = false, model.caddr)
...
proc main() =
  ...
  cfg.title = "06-CoordinateSystems/Exercise3"
  cfg.resizable = false
