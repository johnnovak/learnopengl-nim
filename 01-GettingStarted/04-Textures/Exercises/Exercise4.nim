# Use a uniform variable as the mix function's third parameter to vary the
...
const TEX_MIX_FACTOR_DELTA = 0.02

var texMixFactor = GLfloat(0.3)

var vertices = [
  # Positions               # Texture Coords
  GLfloat(0.5), 0.5, 0.0,   1.0, 1.0,  # Top Right
...
proc setup() =
  shaderProgram = createShaderProgramFromFile("exercise4.vs",
                                              "exercise4.fs")
...
proc draw() =
  ...
  # Set texture mix factor
  shaderProgram.setUniform1f("texMixFactor", texMixFactor)
...
proc keyCb(w: Win, key: Key, scanCode: int, action: KeyAction,
  ...
  if action != kaUp:
    case key:
    of keyEscape: w.shouldClose = true
    of keyUp:     texMixFactor = min(texMixFactor + TEX_MIX_FACTOR_DELTA, 1.0)
    of keyDown:   texMixFactor = max(texMixFactor - TEX_MIX_FACTOR_DELTA, 0.0)
    else:
      discard
